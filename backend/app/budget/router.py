...
def generate_forecast(payload: ForecastRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    result = forecast_from_artifact_for_department(payload.dept_id, max(1, payload.months_ahead))
    if result is None:
        transactions = (
        ...
        if monthly.empty:
            raise HTTPException(status_code=400, detail="No positive transaction data available")

        result = run_budget_forecast(monthly, max(1, payload.months_ahead))
    ...
    for predicted in result.forecasts:
        period_start = datetime.strptime(str(predicted["month"]), "%Y-%m").date().replace(day=1)
