...
const LoginPage: React.FC<LoginPageProps> = ({ onLogin }) => {
  ...
  const loginApi = async (targetEmail: string, targetPassword: string) => {
    setError(null);
    setIsLoading(true);
    try {
      const data = await api.login(targetEmail, targetPassword);
  ...
  const quickLogin = (targetEmail: string) => {
    ...
    loginApi(targetEmail, 'password123');
  };
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-10">
          ...
          <p className="text-slate-500 mt-3 font-medium">AI-assisted transaction intelligence for departments and auditors</p>
        </div>

        <div className="bg-white rounded-3xl shadow-2xl shadow-slate-200/50 p-8 lg:p-10 border border-slate-100">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-2 tracking-tight">Email</label>
            ...
            <button
              ...
              className={`w-full py-4 rounded-2xl font-black text-white shadow-lg shadow-blue-500/20 transition-all transform active:scale-[0.98] ${
                isFormValid && !isLoading ? 'bg-blue-600 hover:bg-blue-700 hover:-translate-y-0.5' : 'bg-slate-300 cursor-not-allowed'
              }`}
            >
          ...
          <div className="mt-8 pt-8 border-t border-slate-100 flex flex-col items-center gap-4">
            <p className="text-xs text-slate-400 font-bold tracking-widest uppercase">Quick Start (Demo Credentials)</p>
            <div className="flex flex-wrap gap-2 justify-center">
              <button onClick={() => quickLogin('superadmin@finsight.com')} className="px-3 py-1.5 bg-slate-100 hover:bg-blue-50 hover:text-blue-600 rounded-full text-[10px] font-black text-slate-600 transition-all flex items-center gap-1">
                <Play size={10} className="fill-current" /> Super Admin
              </button>
              <button onClick={() => quickLogin('admin@finsight.com')} className="px-3 py-1.5 bg-slate-100 hover:bg-blue-50 hover:text-blue-600 rounded-full text-[10px] font-black text-slate-600 transition-all flex items-center gap-1">
                <Play size={10} className="fill-current" /> Admin
              </button>
              <button onClick={() => quickLogin('employee@finsight.com')} className="px-3 py-1.5 bg-slate-100 hover:bg-blue-50 hover:text-blue-600 rounded-full text-[10px] font-black text-slate-600 transition-all flex items-center gap-1">
